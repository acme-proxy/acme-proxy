...
> [ACME Proxy Signer](../signers/acme_proxy.md#reference). Use a staging endpoint
> while you are still working the configuration out.

## 1. Let's Encrypt Relay with DNS Validation
...
## 2. Public CA Relay with HTTP Validation
...
## 3. Commercial ACME CA with EAB and NetBox Filtering

This scenario uses a commercial CA backend. It enforces External Account Binding (EAB) on the internal proxy so only authorized users can register. It additionally uses NetBox as an IPAM filter to verify if a client's IP is actually permitted to request a certificate for a specific DNS name.
...
# so we bypass the upstream challenge.
