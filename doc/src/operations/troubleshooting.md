...
| `request_timeout_ms` too small | It must exceed `signer.custom.timeout_ms` when that backend is configured — that hook runs *inline* inside a request, so a smaller whole-request deadline would cut it off every time. `challenge.timeout_ms` is **not** checked against it: validation runs in the job queue. | Raise `server.request_timeout_ms`. |
