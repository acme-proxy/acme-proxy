...
//!
//! The shape mirrors `filter`/`signer`: a
...
//! list (defaulting to every kind) decides what reaches it, and the list
...
/// One certificate in a [`NotifyEvent::CertificatesExpiring`] digest.
///
/// `days_remaining` is derived here rather than left to a template: it is what
/// the message is actually about, every backend wants it, and computing it in
/// Jinja from two epoch seconds is arithmetic no template should carry.
///
/// `superseded_by` is the member that makes the digest readable — an operator
/// scans for the entries where it is absent. It is `None` both when nothing
/// has replaced this certificate and when this server cannot be *sure*
/// something has: the annotation is deliberately conservative, since a wrong
/// "already renewed" is an operator ignoring a certificate that really is
/// about to lapse, where a missing one is only noise.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct ExpiringCertificate {
    ...
    pub cert_serial: String,
    pub identifiers: Vec<String>,
    /// The leaf's own notAfter, epoch seconds.
    pub not_after: i64,
    /// Whole days from the sweep to `not_after`, floored, never negative.
...
///
/// The one event carrying a **list**, and the only one with no request, no
/// account and no single certificate behind it — it is generated by
/// [`crate::notify::expiry`], on a schedule, about however many certificates
/// happen to qualify.
///
/// `total` is the number that matched, which `certificates` may be shorter
/// than: `notify.expiry.max_entries` bounds the list, so a template renders
/// "and `total - certificates|length` more". The count is honest even when the
/// list is truncated, which is the whole reason it is a separate member.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct CertificatesExpiringData {
    pub profile: String,
    /// When the sweep ran, epoch seconds — the point `days_remaining` counts
    /// from, so a message read days later is still self-describing.
    pub generated_at: i64,
    /// The window this digest covers, `notify.expiry.lead_days`.
    pub lead_days: u64,
    /// How many certificates matched, before `max_entries` truncated the list.
    pub total: i64,
...
/// inside `include_str!` — across 21 entries here and 35 in
...
pub(crate) mod tests {
    ...
    /// The digest's three templates, which are the first to loop over a list
    /// and the only ones a missing file would break *permanently* — `render`
    /// treats an absent template as a permanent failure, so an event with no
    /// template is a notification that can never be delivered rather than one
    /// that is retried.
    ///
    /// The two entries are the point: one already replaced and one not, so the
    /// annotation is asserted in both directions. A digest that renders every
    /// row the same way would be useless in exactly the way this feature
    /// exists to avoid.
    #[test]
