//! `[logging]` — resolving the configuration into an installed subscriber, and
//! swapping it on a reload.
//!
...
//!
//! Which invocation gets a subscriber at all, and the `--log-level` flag whose
//! directive outranks `RUST_LOG` and `logging.filter`, are the CLI's
//! (`cli::logging`); this module takes that directive as a string and records
//! which of the three layers won in [`FilterSource`], so a reload can say.
//!
//! # Reloading
//!
...
///
/// `flag` outranks `RUST_LOG` for `cli::style`'s reason: it was typed on
...
///
/// `directive` is `None` when a non-empty `RUST_LOG` is what asked, in which case
/// it supplies the filter through [`build_env_filter`]'s second layer — see
/// `cli::plan_logging`, which is what decides this function is called at all.
pub fn init_command_logging(directive: Option<String>) -> Result<(), String> {
    init_logging(&command_logging_config(), directive)
}
...
mod tests {
    ...
    /// ambient — `cli::style`'s argument for `--color always`
    ...
    fn the_flag_outranks_rust_log_and_the_file() {
        ...
        let resolved = build_env_filter(&logging, Some("acme_proxy=trace"))
    ...
    fn a_command_subscriber_installs_at_the_flags_level() {
        ...
        init_command_logging(Some("acme_proxy=warn".to_string())).expect("the subscriber installs");
    ...
    fn the_flag_survives_a_reload() {
        ...
        init_logging(&at_error, Some("acme_proxy=debug".to_string()))
            .expect("the subscriber installs");
        assert_eq!(LevelFilter::current(), LevelFilter::DEBUG);
