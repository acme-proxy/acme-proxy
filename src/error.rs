...
/// Declares the problem constructors that are nothing but a status, a type URN
/// and the caller's detail — which is 27 of the 29.
///
/// They were written out longhand, six lines each, and the repetition was the
/// point of failure: a wrong `StatusCode` or a typo'd URN in one of twenty-odd
/// near-identical bodies reads as correct code. Here each is one line against
/// its own doc comment, and the shared body exists once.
///
/// The two that are absent stay hand-written because they are not this shape:
/// [`Problem::compound`] takes its status from the caller, and
/// [`Problem::bad_signature_algorithm`] carries the `algorithms` member RFC 8555
/// §6.2 requires.
